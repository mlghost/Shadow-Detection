...
class IlluminatorNet:
    ...
    def build_net(self):
        ...
        with tf.name_scope('IlluminatorNet'):
            ...
            tf.summary.image('Logits', tf.reshape(tf.nn.softmax(mask)[..., 1], [-1, 224, 224, 1]))
    ...
    def train(self):
        ...
        with tf.name_scope('Model'):
                ...
                #
                # FOR = FN / (FN + TN)
                softmax_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=mask_log,labels= self.mask))
                #
        ...
        with tf.Session() as sess:
            ...
            saver.restore(sess,self.weight_save + 'Model-' + str(5000)+'.ckpt')
            ...
            for ep in range(self.EPOCH):
                ...
                for bn in range(int(len(names) / self.BATCH_SIZE)):
                    ...
                    if (epoch % 1000) == 0:
                        saver.save(sess, self.weight_save + 'MModel-' + str(epoch) + '.ckpt')
                        print 'Model was saved successfully in epoch ' + str(epoch)
                    epoch += 1


if __name__ == '__main__':
    model = IlluminatorNet(BATCH_SIZE=10)
    model.train()
