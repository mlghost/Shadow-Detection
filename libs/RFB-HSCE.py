...
class IlluminatorNet:
    ...
    def build_net(self):
        ...
        with tf.name_scope('IlluminatorNet'):
            ...
            pixel_scaling = max(1, 255 // 2)

            predictions = tf.expand_dims(tf.argmax(mask, 3), -1)
            summary_predictions = tf.cast(predictions * pixel_scaling, tf.uint8)
            tf.summary.image('Shadow Mask-predicted', summary_predictions)

            sm = tf.expand_dims(tf.argmax(self.mask, 3), -1)
            sm_predictions = tf.cast(sm * pixel_scaling, tf.uint8)
            tf.summary.image('Shadow Mask', sm_predictions)
    ...
    def train(self):
        ...
        with tf.name_scope('Model'):
            ...
            d = tf.nn.softmax(mask_log, -1)
            sc = hsce(mask_log, self.mask)
            # sig = tf.nn.softmax(mask_log,-1)
            # positive_shadow_mask = tf.cast(self.mask[..., 1], tf.bool)
            # negative_shadow_mask = tf.logical_not(positive_shadow_mask)
            #
            # fp = tf.reduce_mean(tf.metrics.false_positives(self.mask,mask_log))
            # tp = tf.reduce_mean(tf.metrics.true_positives(self.mask, mask_log))
            # tn = tf.reduce_mean(tf.metrics.true_negatives(self.mask, mask_log))
            # fn = tf.reduce_mean(tf.metrics.false_negatives(self.mask, mask_log))
            #
            # BER = 1 - 0.5 *(tp/(tp+fn) + tn/(tn+fp))
            # tf.summary.scalar('BER',BER)

            # lightness = tf.reduce_mean(self.himage, -1)
            # darkness_loss = tf.reduce_mean(tf.exp(10 * self.mask[..., 0] * (1-lightness)) * self.mask[..., 0] * mask_log[..., 1]) ** 2
            # lightness_loss = tf.reduce_mean(tf.exp(5 * self.mask[..., 1] * lightness) * self.mask[..., 1] * mask_log[..., 0]) ** 2
            # sm = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(self.mask, mask_log))
            # sc = sm + 10 * darkness_loss + 5 * lightness_loss

            # auc = tf.reduce_mean(tf.metrics.auc(self.mask,tf.sigmoid(mask_log)))
        ...
        with tf.Session() as sess:
            ...
            if istrain:
                ...
                import matplotlib.pyplot as plt
                for image_name in os.listdir(self.test_image_path):
                    im = sess.run(d, feed_dict={self.himage: [mp.imread(self.test_image_path + image_name)]})[0]
                    plt.imshow(im[..., 1])
                    plt.show()


if __name__ == '__main__':
    model = IlluminatorNet(BATCH_SIZE=1, sd_lr=0.00001)
    model.train()
