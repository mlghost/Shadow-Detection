...
class IlluminatorNet:
    ...
    def build_net(self):
        ...
        with tf.name_scope('IlluminatorNet'):
            ...
            removed_tensor = RFUNet([f1, f2, f3, f4], 3, 'ShadowRemoval')
            tf.summary.image('FakeRemoved', removed_tensor * 255)
            tf.summary.image('Removed', self.removed * 255)

        return mask, removed_tensor

    def train(self):

        with tf.name_scope('Model'):
            mask_log, removed_image = self.build_net()

            positive_shadow_mask = tf.cast(self.mask[..., 1], tf.bool)
            negative_shadow_mask = tf.logical_not(positive_shadow_mask)
            #
            # FP = tf.reduce_mean(tf.boolean_mask(mask_log[..., 1], negative_shadow_mask))
            # TP = tf.reduce_sum(tf.boolean_mask(mask_log[..., 1], positive_shadow_mask))
            #
            # FDR = FP / (FP + TP)
            #
            # FN = tf.reduce_mean(tf.boolean_mask(mask_log[..., 0], positive_shadow_mask))
            # TN = tf.reduce_sum(tf.boolean_mask(mask_log[..., 0], negative_shadow_mask))
            #
            # FOR = FN / (FN + TN)
            softmax_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(self.mask, mask_log))
            # softmax_loss = lovasz_softmax(mask_log, self.mask, per_image=True)
            reg_loss = tf.add_n(
                [0.00005 * tf.nn.l2_loss(var) for var in tf.trainable_variables() if 'conv5' not in var.name and
                 'fc' not in var.name])

            # Removal Cost
            shadow_area = tf.boolean_mask(removed_image, positive_shadow_mask)
            non_shadow_area = tf.boolean_mask(removed_image, negative_shadow_mask)
            shadow_area_ = tf.boolean_mask(self.removed, positive_shadow_mask)
            non_shadow_area_ = tf.boolean_mask(self.removed, negative_shadow_mask)

            removal_loss1 = tf.losses.mean_squared_error(shadow_area_, shadow_area)
            removal_loss2 = tf.losses.mean_squared_error(non_shadow_area_, non_shadow_area)
            removal_loss = removal_loss1 + removal_loss2 * 30

            loss = softmax_loss + reg_loss + removal_loss
        ...
        with tf.Session() as sess:
            ...
            # saver.restore(sess,self.weight_save + 'Model-' + str(20)+'.ckpt')
            ...
            for ep in range(self.EPOCH):
                ...
                for bn in range(int(len(names) / self.BATCH_SIZE)):
                    ...
                    rhsv = [mp.imread(self.removed_path + name) / 255 for name in loc_name]
...
if __name__ == '__main__':
    model = IlluminatorNet(BATCH_SIZE=6)
    model.train()
